...
!!! Note: 
    If you only plan on using the `oxd-server`, oxd needs to be installed on the same server(s) as the application(s) you want to secure. If you plan on enabling the `oxd-https-extension`, oxd can be installed on any server or VM with network access.
