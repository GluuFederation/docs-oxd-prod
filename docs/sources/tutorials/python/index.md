...
For this tutorial, our OpenID Connect Provider (OP) is Gluu Server 4.3. We are also using oxd 4.3 and an https & cgi enabled web server -- Apache in our case. 

### Gluu Server 4.3 (OP)
As stated above, in this tutorial we're using Gluu Server 4.3 as the OP. 

- Follow [these instructions](https://gluu.org/docs/ce/4.3/installation-guide/install/) to install Gluu Server 4.3. In this tutorial, I installed Gluu Server on host **op.server.com**.

- Add a test user. I added user `test_user`.

### oxd Server 4.3
To install oxd Server 4.3.1, follow [these instructions](../../install/index.md). 
