...
- Only register and allow ongoing access to users with one or more of the following roles in the OP: Using this option you can limit registration to users that have a specified role in the OP, for instance `Drupal`. Each time the user authenticates they will need to have this scope present in order to be approved for access (i.e. if you remove this scope from the users profile in the OP, the user would be denied access);
...
- Disable automatic registration: If you choose to disable automatic registration, you will need to manually add a user in Drupal for each person that needs access. Make sure that when you add the user in Drupal, you use the same email they have registered in the OP. 
...
2. Add a name for the role want to use to enforce access (e.g. `Drupal` or `website`);     
